...
export const ProfileView = () => {
  ...
  const { user,
    ...
    message, errors: userErrors } = useAuthStore();
  ...
  const onUpload = ({ target }) => {
    ...
    startUploadingProfilePicture(target.files[0])
      .then(() => {
  ...
  const onSubmit = (data) => {
    ...
    startUpdateAuthProfile({ fullName, email })
      .then(() => {
    ...
    <div className="row">
      ...
      <div className="col-md-4">
        ...
        <div className="col-md-12 text-center">
          {
            isLoadingPicture ?
              (<SpinnerLoader />) :
              (<img className="rounded-circle img-fluid profile-picture" src={user.profileUrl} alt="user-photo" />)
          }
        </div>
        <input
          type="file"
        ...
        <span className="float-right p-2">
          <small className="text-secondary mr-2" style={{ fontSize: "12px" }}>Tamaño máximo: 1MB.</small>
          <FontAwesomeIcon
            style={{ cursor: "pointer" }}
            onClick={() => fileInputRef.current.click()} icon={faUpload} />
        </span>
      </div>
